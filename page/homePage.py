...
from base.tableOperation import TableOperation
import page
...
class HomePage(LoginPage):
    ...
    def search_input_element(self):
        """检查输入框"""
        return self.base_find(page.calc_home_input)

    def search_button_element(self):
        """检查按钮"""
        return self.base_find(page.calc_home_search)

    # 按钮
    def add_button_element(self):
        """新增按钮"""
        return self.base_find(page.calc_home_add)

    def edit_button_element(self):
        """编辑按钮"""
        return self.base_find(page.calc_home_edt)

    def delete_button_element(self):
        """删除按钮"""
        return self.base_find(page.calc_home_del)

    # 编辑弹窗
    def edit_code_element(self):
        """学号输入框"""
        return self.base_find(page.calc_home_add_code)

    def edit_name_element(self):
        """姓名输入框"""
        return self.base_find(page.calc_home_add_name)

    def edit_sex_element(self):
        """性别输入框"""
        return self.base_find(page.calc_home_add_sex)

    def edit_grader_element(self):
        """年级输入框"""
        return self.base_find(page.calc_home_add_grader)

    def edit_confirm_button_element(self):
        """编辑确认按钮"""
        return self.base_find(page.calc_home_add_confirm)

    def edit_cancel_button_element(self):
        """编辑取消按钮"""
        return self.base_find(page.calc_home_add_cancel)

    # 删除弹窗
    def del_confirm_button_element(self):
        """编辑确认按钮"""
        return self.base_find(page.calc_home_del_confirm)

    def del_cancel_button_element(self):
        """学号输入框"""
        return self.base_find(page.calc_home_del_cancel)
    ...
    def edit_dialog(self, code=None, name=None, sex=None, grader=None, button="确定"):
        ...
        if button == "确定":
            ...
            print("编辑弹窗中按钮只能是确实或者取消")

    def add_data(self, code, name, sex=None, grader=None, button="确定"):
        """
        由于code，name为必输项，所以一定要接受参数
        但是sex，grader为非必输项，所以可以不用传值，默认参数设置为None
        :param code:学号，必输项
        :param name:姓名，必输项
        :param sex:性别，非必输
        :param grader:年级，非必输
